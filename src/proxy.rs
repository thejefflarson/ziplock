...
async fn resolve_host(resolver: &TokioResolver, host: &str) -> Result<Vec<IpAddr>, String> {
    if let Ok(ip) = host.parse::<IpAddr>() {
        ...
        // dev server, etc. is normal developer work here, not an SSRF surface. Public
        // literals stay blocked so public names are forced through the DoH malware filter.
        if is_private_ip(&ip) {
            return Ok(vec![ip]);
        }
